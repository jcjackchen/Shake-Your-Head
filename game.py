...
def action(action_buffer):
    if len(action_buffer) > 0:
        diff_x,diff_y = action_buffer[0]
        del action_buffer[0]
...
def run():
    ...
    while running:
        ...
        if counter % 15 == 0:
            t = threading.Thread(target=determine_action)
            t.start()
        ...
        if getnum < threadnum:
            (diff_x, diff_y) = threads[getnum]
            getnum += 1
        

        # 处理键盘事件（移动飞机的位置）
        if key_pressed[K_w] or key_pressed[K_UP] or diff_y < -10:
            player.moveUp()
        if key_pressed[K_s] or key_pressed[K_DOWN] or diff_y > 10:
            player.moveDown()
        if key_pressed[K_a] or key_pressed[K_LEFT] or diff_x > 10:
            player.moveLeft()
        if key_pressed[K_d] or key_pressed[K_RIGHT] or diff_x < -10:
            player.moveRight()
